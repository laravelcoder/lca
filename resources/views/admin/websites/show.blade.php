...
<li role="presentation" class="active"><a href="#adwords" aria-controls="adwords" role="tab" data-toggle="tab">Adwords</a></li>
...
<div role="tabpanel" class="tab-pane active" id="adwords">
            ...
            @foreach ($adwords as $adword)
                <tr data-entry-id="{{ $adword->id }}">
                    <td field-key='client_customer_id'>{{ $adword->client_customer_id }}</td>
                                ...
                                <td>
                                    @can('view')
                                    <a href="{{ route('adwords.show',[$adword->id]) }}" class="btn btn-xs btn-primary">@lang('quickadmin.qa_view')</a>
                                    @endcan
                                    @can('edit')
                                    <a href="{{ route('adwords.edit',[$adword->id]) }}" class="btn btn-xs btn-info">@lang('quickadmin.qa_edit')</a>
                                    @endcan
                                    @can('delete')
{!! Form::open(array(
                                        'style' => 'display: inline-block;',
            ...
            @foreach ($analytics as $analytic)
                <tr data-entry-id="{{ $analytic->id }}">
                    <td field-key='view_name'>{{ $analytic->view_name }}</td>
                                ...
                                <td>
                                    @can('view')
                                    <a href="{{ route('analytics.show',[$analytic->id]) }}" class="btn btn-xs btn-primary">@lang('quickadmin.qa_view')</a>
                                    @endcan
                                    @can('edit')
                                    <a href="{{ route('analytics.edit',[$analytic->id]) }}" class="btn btn-xs btn-info">@lang('quickadmin.qa_edit')</a>
                                    @endcan
                                    @can('delete')
{!! Form::open(array(
                                        'style' => 'display: inline-block;',
        ...
        @endif
            ...
            <a href="{{ route('admin.websites.index') }}" class="btn btn-default">@lang('quickadmin.qa_back_to_list')</a>
        </div>
    </div>
@stop
