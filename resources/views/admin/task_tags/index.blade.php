...
@section('content')
    ...
    @can('task_tag_create')
    <p>
        <a href="{{ route('admin.task_tags.create') }}" class="btn btn-success">@lang('quickadmin.qa_add_new')</a>
                        ...
                        @foreach ($task_tags as $task_tag)
                            <tr data-entry-id="{{ $task_tag->id }}">
                                ...
                                <td field-key='name'>{{ $task_tag->name }}</td>
                                                                <td>
                                    @can('task_tag_view')
                                    <a href="{{ route('admin.task_tags.show',[$task_tag->id]) }}" class="btn btn-xs btn-primary">@lang('quickadmin.qa_view')</a>
                                    @endcan
                                    @can('task_tag_edit')
                                    <a href="{{ route('admin.task_tags.edit',[$task_tag->id]) }}" class="btn btn-xs btn-info">@lang('quickadmin.qa_edit')</a>
                                    @endcan
                                    @can('task_tag_delete')
{!! Form::open(array(
